		...
		if(pl.owner is playerEmpire) {
			...
			if(scTrait is null || !playerEmpire.hasTrait(scTrait.id)) {
				...
				if(int(surface.totalPressure) > int(surface.pressureCap))
					color = colors::Red;
				string value = standardize(surface.totalSaturate, true) + "/" + standardize(surface.totalPressure, true) + "/" + standardize(surface.pressureCap, true);
				string ttip = format(locale::PLANET_PRESSURE_TIP, standardize(surface.totalSaturate, true), standardize(surface.totalPressure, true), standardize(surface.pressureCap, true));
