...
class Quickbar : BaseGuiElement {
	...
	void refresh() {
		...
		add(LaborPlanets(this));
		add(DefenseTargets(this));
...
class DisabledResources : ObjectMode {
	...
	bool filter(ObjectData@ dat) {
		...
		if(!dat.resource.usable) {
			...
			if(!dat.obj.inCombat && pop < getPlanetLevelRequiredPop(dat.obj.resourceLevel))
				return false;
...
class NoPopResources : ObjectMode {
	...
	bool filter(ObjectData@ dat) {
		...
		if(type is null)
			return false;
		if(!dat.resource.usable) {
			if(dat.obj.population >= getPlanetLevelRequiredPop(dat.obj.resourceLevel))
				return false;
			if(dat.obj.inCombat)
				return false;
			return true;
...
class ArtifGrid : GuiIconGrid {
	...
	void drawElement(uint index, const recti& pos) override {
		...
		//Draw activation energy cost
		if(desc.ability !is null && desc.ability.type !is null) {
			auto@ abl = desc.ability;
			col = Color(0xffffffff);
			int activationCost = abl.getEnergyCost();
			if(activationCost > playerEmpire.EnergyStored)
				col = Color(0xff0000ff);
			gui_root.skin.getFont(FT_Small).draw(horizAlign=0, vertAlign=0, pos=pos,
			                                     text=toString(activationCost), color=col);
		}
