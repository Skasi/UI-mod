...
class StarPopup : Popup {
	...
	StarPopup(BaseGuiElement@ parent) {
		...
		@temperature = GuiText(this, Alignment(Left+6, Top+92, Right-6, Top+114));
		temperature.horizAlign = 1;
	...
	void update() {
		...
		//Update temperature
		temperature.text = toString(obj.temperature, 0)+" K";
		if(ft.getDimension(temperature.text).x > temperature.size.width)
			temperature.font = FT_Detail;
		else
			temperature.font = FT_Normal;
