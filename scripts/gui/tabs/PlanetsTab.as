	...
	void drawTree(const vec2i& startPos) {
		...
		if(isPrimary) {
			...
			if(numTotal != -1) {
				...
				// Tiles view
				if(mode == PM_TILES) {
					txt = format("$1/$2", toString(numUsed), toString(numTotal));
					if(numTotal < 50)
						@ft = font::DroidSans_8;
					else if(numTotal < 90)
						@ft = font::DroidSans_10;
				...
				// Pressure view
					...
					if(numActive != 0 || numUsed != 0 || numTotal != 0)
						txt = format("$1/$2/$3", toString(numActive), toString(numUsed), toString(numTotal));
					if(numUsed < numTotal)
						baseColor = colors::Green;
					else
		...
		if(progressIndex < list.length) {
			...
			for(uint n = 0; n < count; ++n) {
				...
				for(uint i = 0, cnt = resources.length; i < cnt; ++i) {
					...
					ele.showChildren = focusObject is null || ele.obj is focusObject;
