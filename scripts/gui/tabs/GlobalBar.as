...
class InfluenceResource : ResourceDisplay {
	...
	string get_tooltip() {
		double influenceEfficiency = playerEmpire.InfluenceEfficiency;
		double netInfluenceIncome = playerEmpire.InfluenceIncome;
		double influenceIncome = netInfluenceIncome / influenceEfficiency;
		
		array<string> influenceNumbers = {
			toString(playerEmpire.Influence),
			toString(playerEmpire.InfluenceCap, 0),
		
			formatIncomeRate(influenceIncome, perMinute=true),
			"-"+toString((1.0-influenceEfficiency)*100.0, 0)+"%",
			formatIncomeRate(netInfluenceIncome, perMinute=true),
		
			toString(playerEmpire.getInfluenceStock(), 0),
			toString(playerEmpire.InfluencePercentage*100.0, 0)+"%"
		};
		
		return format(locale::GTT_INFLUENCE, influenceNumbers);
	}
...
class ResearchResource : ResourceDisplay {
	...
	string get_tooltip() {
		double netIncome = playerEmpire.ResearchRate;
		double factor = playerEmpire.ResearchEfficiency;
		double income = netIncome / factor;
		
		string tt = format(locale::GTT_RESEARCH,
				toString(playerEmpire.ResearchPoints, 0),
				toString(playerEmpire.ResearchGenerated, 0),
				formatIncomeRate(income),
				"-"+toString((1.0-factor)*100.0, 0)+"%",
				formatIncomeRate(netIncome));
