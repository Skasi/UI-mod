...
void drawCardIcon(const InfluenceCard@ card, const recti& pos, int uses = 0) {
	...
	//Draw costs
	int purchaseCost = card.getPurchaseCost(playerEmpire);
	int playCost     = card.getPlayCost(null, null);
	if(purchaseCost > 0 && purchaser is null) {
		Color col = Color(0xffffffff);
		if(purchaseCost > playerEmpire.Influence)
			col = Color(0xff0000ff);
		gui_root.skin.getFont(FT_Normal).draw(horizAlign=0.1, vertAlign=0.1, pos=pos,
		                                      text=toString(purchaseCost), color=col);
	}
	if(playCost > 0 && purchaser is null) {
		gui_root.skin.getFont(FT_Small).draw(horizAlign=0.9, vertAlign=0.9, pos=pos,
	                                         text=toString(playCost));
	}

	//Draw purchaser
	if(purchaser !is null)
		//print("purchaser !is null");
		spritesheet::ContextIcons.draw(1, iconPos, purchaser.color);
}
