...
final class SurfaceComponent : Component_SurfaceComponent, Savable {
	...
	void replaceFirstBiomeWith(Object& obj, uint id) {
		...
		uint8 prevId = uint8(biomes[0].id);
	...
	float get_totalSaturate() const {
		return grid.totalSaturate;
	}
	...
	void updateLoyalty(Object& obj, double time) {
		...
		double loyMod = time * baseLoyalty / loyTimer;
		...
		if(!protect && !isRelief) {
			...
			for(uint n = 0, cnt = fleetsInOrbit.length; n < cnt; ++n) {
				...
				Empire@ otherOwner = fleetsInOrbit[i].owner;
				if(owner.isHostile(otherOwner) && !isProtected(obj, otherOwner)) {
					...
					if(ship !is null)
						ship.consumeSupply(loyMod * config::SIEGE_LOYALTY_SUPPLY_COST);
					LoyaltyEffect[otherOwner.index] = clamp(LoyaltyEffect[otherOwner.index] - loyMod, -baseLoyalty, 0.0);
					deltaLoy = true;
				}
			}
		}
	...
	void updateDecayLevel(Object& obj, double time, bool wasManual = false) {
		...
		uint TargetLevel = ResourceLevel;
		bool isInstant = false;
		while(TargetLevel > 0 && needsPopulationForLevel) {
			if(Population < getPlanetLevelRequiredPop(TargetLevel) - 0.001) {
				TargetLevel -= 1;
			}
			else {
				break;
		...
		if(newPopLevel != PopLevel) {
			const PlanetLevel@ prevLevel = getPlanetLevel(PopLevel);
			const PlanetLevel@ newLevel = getPlanetLevel(newPopLevel);
	...
	void setLevel(Object& obj, uint lvl, bool first = false, bool wasManual = false) {
		...
		const PlanetLevel@ prevLevel = getPlanetLevel(Level);
		const PlanetLevel@ newLevel = getPlanetLevel(lvl);
		Region@ region = obj.region;
	...
	void changeSurfaceOwner(Object& obj, Empire@ prevOwner) {
		const PlanetLevel@ lvl = getPlanetLevel(obj.level);
		Region@ region = obj.region;
	...
	double get_colonyShipAccel(const Object& obj) {
		return colonyshipAccel * COLONYSHIP_BASE_ACCEL * obj.owner.ModSpeed.value;
	}
	...
	bool get_canSafelyColonize(const Object& obj) const {
		...
		auto@ lv = getPlanetLevel(ResourceLevel);
		...
		growthFactor *= getPlanetLevel(min(Level,obj.primaryResourceLimitLevel)).popGrowth;
		if(obj.inCombat)
			growthFactor = 0;
	...
	void calculatePopVars(Object& obj) {
		...
		for(uint i = 0; i <= MAX_PLANET_LEVEL && pop > 0; ++i) {
			const PlanetLevel@ level = getPlanetLevel(i);
		...
		if(pop > 0) {
			const PlanetLevel@ level = getPlanetLevel(MAX_PLANET_LEVEL);
			baseCap += pop * 3.0;
			income +=  pop * 30.0;
		}
	...
	void surfaceTick(Object& obj, double time) {
		...
		if(Population >= 0.9999 && Population < localMax && bombardDecay <= 0 && MaxPopulation > 1) {
			...
			for(; debtFactor > 0; debtFactor -= 1.f)
				growthFactor *= 0.33f + 0.67f * (1.f - min(debtFactor, 1.f));
			if(needsPopulationForLevel)
				growthFactor *= getPlanetLevel(min(Level,obj.primaryResourceLimitLevel)).popGrowth / 60.0;
			else
				growthFactor *= getPlanetLevel(Level).popGrowth / 60.0;
			growthFactor *= obj.owner.PopulationGrowthFactor;
		...
		int newIncome = ceil(grid.resources[TR_Money] * TILE_MONEY_RATE) + popIncome + bonusIncome;
		...
		double newDefense = max(grid.resources[TR_Defense], 0.0) * DEFENSE_LABOR_PM / 60.0;
		...
		double labor = laborRes * TILE_LABOR_RATE;
	...
	void _writePop(Message& msg) {
		...
		msg.writeLimited(ResourceLevel,MAX_PLANET_LEVEL);
		msg.writeBit(isSendingColonizers);
	}

	void _writeRes(Message& msg) {
		msg.writeLimited(Level,MAX_PLANET_LEVEL);
		if(DecayLevel < Level) {
			msg.write1();
