...
class SurfaceComponent : Component_SurfaceComponent {
	...
	bool get_canSafelyColonize(const Object& obj) const {
		...
		growthFactor *= getPlanetLevel(min(Level,obj.primaryResourceLevel)).popGrowth;
		if(obj.inCombat)
			growthFactor = 0;
	...
	void _readPop(Message& msg) {
		...
		ResourceLevel = msg.readLimited(MAX_PLANET_LEVEL);
	...
	void _readRes(Object& obj, Message& msg, bool initial = false) {
		...
		Level = msg.readLimited(MAX_PLANET_LEVEL);
		if(msg.readBit()) {
